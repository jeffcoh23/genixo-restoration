class IncidentsController < ApplicationController
  ...
  def show
    ...
    render inertia: "Incidents/Show", props: {
      ...
      # Optional — never loaded on the initial page response; fetched per-tab
      # via `router.reload({ only: [...] })` in Show.tsx's handleTabChange.
      labor_log: InertiaRails.optional { serialize_labor_log(@incident) },
      assignable_labor_users: InertiaRails.optional { can_manage_labor? ? assignable_labor_users(@incident) : [] },
      equipment_log: InertiaRails.optional { serialize_equipment_log(@incident) },
      equipment_types: InertiaRails.optional { can_manage_equipment? ? equipment_types_for_incident(@incident) : [] },
      equipment_items_by_type: InertiaRails.optional { can_manage_equipment? ? equipment_items_by_type(@incident) : {} },
      attachable_equipment_entries: InertiaRails.optional { can_manage_activities? ? attachable_equipment_entries(@incident) : [] },
      messages: InertiaRails.optional { serialize_messages(@incident.messages.includes({ user: :organization }, { attachments: [ :uploaded_by_user, { file_attachment: :blob } ] }).order(created_at: :asc)) },
      attachments: InertiaRails.optional { serialize_attachments(@incident) },
      operational_notes: InertiaRails.optional { serialize_operational_notes(@incident) },
      activity_entries: InertiaRails.optional { serialize_activity_entries(@incident) },
      moisture_data: InertiaRails.optional { serialize_moisture_data(@incident) },
      psychrometric_data: InertiaRails.optional { serialize_psychrometric_data(@incident) },
      assignable_mitigation_users: InertiaRails.optional { can_assign_to_incident? ? assignable_incident_users(@incident, :mitigation) : [] },
      assignable_pm_users: InertiaRails.optional { can_assign_to_incident? ? assignable_incident_users(@incident, :pm) : [] }
    }
  end
