...
- Anyone can submit the public `/request-access` form (rate-limited: 5/min per IP). It creates a `LoginRequest` — never an account. The requester **types their company as free text** (`company_name`) — the public form must never render a dropdown of client orgs, since that exposes the customer list on an unauthenticated page. Scalar fields are trimmed and stripped of control characters (prevents fake-line injection into the reviewer email).
...
- Review happens on the Users page (MANAGE_USERS required):
  - **Approve** marks the request approved and opens the invite modal prefilled with the requester's details, but with **org and role cleared** — the admin deliberately matches the typed company to a real org (a stale/default preselection could invite the requester into the wrong org). Blank org is also rejected server-side. Account creation always goes through the normal invitation flow. (Legacy dropdown-era rows carrying a serviceable `organization_id` still prefill org + a default Property Manager role.)
